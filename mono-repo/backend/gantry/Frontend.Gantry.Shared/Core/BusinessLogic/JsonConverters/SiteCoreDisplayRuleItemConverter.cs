        ...
        public override object? ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            if (siteCoreItemDetailsMessageInArray != null)
            {
                siteCoreDisplayRuleItemDetails = new SiteCoreDisplayRuleItemDetails
                {
                    ...
                    ContentMediaType = string.IsNullOrEmpty(siteCoreItemDetailsMessageInArray?.ContentMediaType?.FirstOrDefault()) ? ConstantsPropertyValues.Simple : siteCoreItemDetailsMessageInArray?.ContentMediaType?.FirstOrDefault(),
                    TradingPartitionId = siteCoreItemDetailsMessageInArray?.TradingPartitionId?.FirstOrDefault(),
                    RacingAssetType = siteCoreItemDetailsMessageInArray?.RacingAssetType?.FirstOrDefault(),
                    SplitScreen = siteCoreItemDetailsMessageInArray?.SplitScreen != null ? JsonConvert.DeserializeObject<SplitScreen>(siteCoreItemDetailsMessageInArray.SplitScreen?.FirstOrDefault()) : null,
                    ContentProvider = string.IsNullOrEmpty(siteCoreItemDetailsMessageInArray?.ContentProvider?.FirstOrDefault()) ? ConstantsPropertyValues.ContentProviderDTP : siteCoreItemDetailsMessageInArray?.ContentProvider?.FirstOrDefault(),
                };
            }
