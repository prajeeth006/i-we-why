        ...
        public async Task ProcessPresenceMessageNew(PresenceMessage presenceMessage)
        {
            try
            {
                ...
                if (presenceMessage?.Sender?.groups != null)
                {
                        ...
                        if (matchedDisplayRuleForEachScreenInShop?.Count > 0)
                        {
                            ...
                            foreach (var rule in matchedDisplayRuleForEachScreenInShop)
                            {
                                if (rule != null)
                                {
                                    ...
                                    bool validateScreenGroup = KafkaMessageHelper.ValidateScreenGroups(rule, replacedPlaceHolderInUrl);
                                    if (validateScreenGroup)
                                    {
                                        screenGroups.Add(new ScreenGroup
                                        {
                                            ...
                                            viewId = rule.ViewId,
                                            ContentMediaType = rule.ContentMediaType,
                                            ContentProvider = rule.ContentProvider
                                        });
                                    }
                                    else
                                    {
                                        _log.LogError($"Presence message SID: {presenceMessage.Sid} There are no rule items in rule object  {JsonConvert.SerializeObject(rule)}, TemplateUrl: {replacedPlaceHolderInUrl}");
                                    }
                    ...
                    bool validateMessageDetailsMessage = KafkaMessageHelper.ValidationProduceMessageToKafka(rtmsMessageDetailsMessage);
                    ...
                    {
                        _log.LogError($"Presence message SID: {presenceMessage.Sid}, rtmsMessageDetailsMessage details :  getting null or empty values, presence message completed with Message: {JsonConvert.SerializeObject(rtmsMessageDetailsMessage)}");
                    }
                }
                else
                {
