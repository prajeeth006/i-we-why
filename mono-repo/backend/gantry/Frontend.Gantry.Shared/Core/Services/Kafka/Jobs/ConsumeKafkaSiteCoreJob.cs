...
namespace Frontend.Gantry.Shared.Core.Services.Kafka.Jobs
{
    public class ConsumeKafkaSiteCoreJob : BaseJob
    {
        ...
        private string _consumerProtocol;


        public ConsumeKafkaSiteCoreJob(
            ISiteCoreKafkaConsumerConfig siteCoreKafkaConsumerConfig,
            ILogger<ConsumeKafkaSiteCoreJob> log, IBrandUrl brandUrl, IGantryBrandUrlMapping gantryBrandUrlMapping, IDisplayManagerScreens displayManagerScreens, IConfiguration configuration, IHostApplicationLifetime hostApplicationLifetime) 
            : base(log)
        {
        ...
        protected override void DoPreWorkBeforeOwnThreadStarts()
        {
            ...
            _consumerProtocol = _siteCoreKafkaConsumerConfig.ConsumerProtocol;
        }

        protected override async Task DoWorkInOwnThread()
        {
            Log.LogInformation("ConsumeKafkaSiteCoreJob: New thread started.");

            Log.LogInformation("ConsumeKafkaSiteCoreJob: Registering IIS appool stopping event to stop this JOB.");
            _hostApplicationLifetime.ApplicationStopping.Register(StoppingDoWork);
            ...
            {
                consumer.Close();
                Log.LogInformation("Consumer for sitecore kafka is closed");
            }
        }

        protected override void StoppingDoWork()
        {
            _shouldKeepRunning = false;
            Log.LogInformation("Consume Sitecore kafka Job is stopped");
        }
        ...
        private async Task SendWithHttpGetRequest(string messageSiteCoreItemDetailsString)
        {
            ...
            try
            {
                    ...
                    //Buidling the URI based on Scheme name using URIBuilder
                    urlBuilder = new UriBuilder(_consumerProtocol, $"{env}{brandUrl}/en/api/ProcessSiteCoreKafkaMessage");
