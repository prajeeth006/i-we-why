...
namespace Frontend.Gantry.Shared.Core.Services.Kafka.Jobs
{
    public class ConsumeKafkaRtmsPresenceJob : BaseJob
    {
        ...
        public ConsumeKafkaRtmsPresenceJob(
            ...
            IRtmsKafkaConsumerConfig rtmsKafkaConsumerConfig, IBrandUrl brandUrl, IGantryBrandUrlMapping gantryBrandUrlMapping, IConfiguration configuration, IHostApplicationLifetime hostApplicationLifetime) 
            : base(log)
        {
        ...
        protected override void DoPreWorkBeforeOwnThreadStarts()
        {
            ...
            _consumerProtocol = _rtmsKafkaConsumerConfig.ConsumerProtocol;
        }
        ...
        protected override async Task DoWorkInOwnThread()
        {
            ...
            Log.LogInformation("ConsumeKafkaRtmsPresenceJob: Registering IIS appool stopping event to stop this JOB.");
            ...
            {
                consumer.Close();
                Log.LogInformation("RtmsKafkaPresenceAndAckService: Consumer is closed");
            }
        }
        ...
        protected override void StoppingDoWork()
        {
            ...
            Log.LogInformation("RtmsKafkaPresenceAndAckService: Job is stopped");
        }
        ...
        private async Task SendWithHttpGetRequest(string presenceMessageParam)
        {
            ...
            try
            {
                ...
                if (presenceMessage.Sender.device.ToLower().Contains(ConstantsPropertyValues.RtmsPresenceMessageDeviceId) && presenceMessage.Type?.ToUpper() == ConstantsPropertyValues.RtmAppConnect)
                {
                    ...
                    var urlBuilder = new UriBuilder(_consumerProtocol,$"{env}{brandUrl}/en/api/ProcessPresenceMessage");
