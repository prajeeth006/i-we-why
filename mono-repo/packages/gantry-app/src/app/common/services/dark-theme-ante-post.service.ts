...
import { EMPTY, catchError, map, startWith, tap } from 'rxjs';
...
export class DarkThemeAntePostService {
    ...
    data$ = this.sportBookService.data$.pipe(
    ...
    prepareAntiPost(sportBookEvent: SportBookEventStructured) {
        const result = new AntePostResult();
        if (!sportBookEvent) {
            return result;
        }
        ...
        result.selections.forEach((selection) => {
            const prices = SportBookMarketHelper.getPreparePrice(SportBookSelectionHelper.getLatestPrice(selection));
            const selectionName = StringHelper.selectionNameLengthAndTrimEnd(selection?.selectionName, SelectionNameLength.Eighteen);
        ...
        result.outRightTitle = result?.isOutrightResults
            ? result.event?.categoryName
            : (result.racingContent?.contentParameters?.Title ?? result.event?.categoryName);
    ...
    private prepareResult(result: AntePostResult) {
        if (result?.market) {
            result.market.marketName = result?.market?.marketName?.replace(' ', '');
            result.marketEachWay = SportBookMarketHelper.prepareEachWayString(result?.market, this.gantryCommonContent);
        }
