import { GreyhoundMeetingRunnerDetails } from '../../greyhound-racing/models/greyhound-racing-meeting-results.model';
import { Positions } from '../../greyhound-racing/models/greyhound-racing.enum';
...
export class RunnerDetailsRacingEvent {
    static setRunnerDetails(runnerType: string, runnerCount: number, eachWay?: string, forecast?: string, tricast?: string, runnerList?: any) {
        if (runnerType) {
            const eachWayList = this.setEachWayDetails(eachWay);
            const eachWayFraction = eachWayList[0];
            const eachWayPlaces = eachWayList[1];
            const eachWayLastPlace = eachWayList[2];
            if (runnerType?.toUpperCase() == RunnerType.Horse || runnerType?.toUpperCase() == RunnerType.Motor) {
                runnerList = this.setRunnerDetailsForHorseRace(runnerCount, eachWayFraction, eachWayPlaces, eachWayLastPlace, tricast, runnerList);
            } else if (runnerType?.toUpperCase() == RunnerType.Dog) {
                runnerList = this.setRunnerDetailsForDogRace(runnerCount, eachWayFraction, eachWayPlaces, eachWayLastPlace, runnerList);
            }
        }
        return runnerList;
    }
    ...
    static setRunnerDetailsBasedOnSortedTricast(runnerType?: string, eachWay?: string, runnerList?: any, sortedTricast?: any) {
        if (runnerType) {
            if (runnerType?.toUpperCase() == RunnerType.Horse || runnerType?.toUpperCase() == RunnerType.Motor) {
                runnerList = runnerList?.sort(
                    (a: any, b: any) => Number(a.position) - Number(b.position) || Number(a.horseRunnerNumber) - Number(b.horseRunnerNumber),
                );
            } else if (runnerType?.toUpperCase() == RunnerType.Dog) {
                runnerList = runnerList?.sort(
                    (a: any, b: any) => Number(a.position) - Number(b.position) || Number(a.runnerNumber) - Number(b.runnerNumber),
                );
            }
            const eachWayList = this.setEachWayDetails(eachWay);
            const eachWayFraction = eachWayList[0];
    ...
    private static setRunnerDetailsForHorseAndDog(
        runnerType?: string,
        eachWayFraction?: string,
        eachWayPlaces?: string,
        eachWayLastPlace?: string,
        runnerList?: any,
        sortedTricast?: any,
    ) {
        let results = [];
        try {
            const pos1Count = runnerList?.filter((x: any) => x.position == '1')?.length;
            const pos2Count = runnerList?.filter((x: any) => x.position == '2')?.length;
            const pos3Count = runnerList?.filter((x: any) => x.position == '3')?.length;
            const pos4Count = runnerList?.filter((x: any) => x.position == '4')?.length;
            if (!sortedTricast || sortedTricast == 0) {
                if (eachWayPlaces == '1') {
                    if (eachWayFraction == '1/1') {
                        if (pos1Count == 1 && pos2Count == 1) {
                            //Senario - A
                        ...
                        } else if (pos1Count == 1 && pos2Count >= 2) {
                            //Senario - C
                            results = runnerList.filter((x: any) => x.position <= 2);
                            this.setForecastForPosition(results, 2);
                        } else {
                    ...
                    if (pos1Count == 1 && pos2Count == 1 && pos3Count == 1) {
                        //Senario - M
                    ...
                    if (pos1Count == 1 && pos2Count == 1 && pos3Count == 1 && pos4Count == 1) {
                        //Senario - Q
                    ...
                    const eachWayMaxPlaces = eachWayLastPlace ? Number(eachWayLastPlace) : 0;
                    results = runnerList.filter((x: any) => x.position <= eachWayMaxPlaces); //Senario - U and Senario - W
                }
            } else {
                if (eachWayPlaces == '1') {
                    console.log('Nothing else to do with this case');
                    ...
                    if (pos1Count == 1 && pos2Count == 1 && pos3Count == 1) {
                    ...
                    } else if (pos1Count == 2 && pos3Count == 1) {
                        //Senario - H
                        results = runnerList.filter((x: any) => x.position == '1' || x.position == '3');
                        this.setTricastForPosition(results, 3);
                    } else if (pos1Count == 2 && pos3Count >= 2) {
                        //Senario - I
                        results = runnerList.filter((x: any) => x.position == '1' || x.position == '3');
                        this.setTricastForPosition(results, 3);
                    } else if (pos1Count >= 3) {
                        ...
                        //Senario - K
                        results = runnerList.filter((x: any) => x.position <= 2);
                    } else if (pos1Count == 1 && pos2Count == 1 && pos3Count >= 2) {
                        //Senario - L
                        results = runnerList.filter((x: any) => x.position <= 3);
                        this.setTricastForPosition(results, 3);
                    } else {
                    ...
                    const eachWayMaxPlaces = eachWayLastPlace ? Number(eachWayLastPlace) : 0;
                    results = runnerList.filter((x: any) => x.position <= eachWayMaxPlaces); //Senario - V and Senario - X
                }
            }
            ...
            if (runnerType?.toUpperCase() == RunnerType.Horse || runnerType?.toUpperCase() == RunnerType.Motor) {
                results = results?.sort(
                    (a: any, b: any) =>
                        (a.position ? -1 : 1) - (b.position ? -1 : 1) ||
                        Number(a.position) - Number(b.position) ||
                        Number(a.horseRunnerNumber) - Number(b.horseRunnerNumber),
                );
            } else if (runnerType?.toUpperCase() == RunnerType.Dog) {
                results = results?.sort(
                    (a: any, b: any) =>
    ...
    private static setRunnerDetailsForHorseRace(
        runnerCount: number,
        eachWayFraction?: string,
        eachWayPlaces?: string,
        eachWayLastPlace?: string,
    ...
    private static setRunnerDetailsForDogRace(
        runnerCount: number,
        eachWayFraction?: string,
        eachWayPlaces?: string,
        eachWayLastPlace?: string,
    ...
    private static setEachWayDetails(eachWay?: string) {
        const eachWayList: Array<string> = [];
        let eachWayFraction = '';
        let eachWayPlaces = '';
        let eachWayLastPlace = '';
        if (eachWay) {
            const terms = eachWay?.split(' ');
            if (terms[1]) {
                eachWayFraction = terms[1];
            }
            eachWayPlaces = terms[2];
            const places = eachWayPlaces?.split('-');
    ...
    private static setRunnerList(runnerListPosition: number, runnerList?: any) {
        ...
        tempList?.sort((a, b) => (a.position ? -1 : 1) - (b.position ? -1 : 1) || Number(a.position) - Number(b.position));

        return tempList;
    }

    private static setDeadHeat(runnerList: GreyhoundMeetingRunnerDetails[], isDeadHeat: boolean = false): boolean {
        const findRunnerPositions: GreyhoundMeetingRunnerDetails[] = [];
        runnerList.forEach((checkPosition, i) => {
            const runner = runnerList?.find(
                (runner: GreyhoundMeetingRunnerDetails, index: number) => i !== index && runner?.position === checkPosition?.position,
            );
            !!runner && findRunnerPositions.push(checkPosition);
        });
        findRunnerPositions?.forEach((runnerPosition) => {
            if (
                runnerPosition?.position == Positions.One ||
                runnerPosition?.position == Positions.Two ||
                runnerPosition?.position == Positions.Three
            ) {
    ...
    private static prepareSortingDeadHeat(runnerList: GreyhoundMeetingRunnerDetails[]) {
        runnerList?.sort(
            (a: any, b: any) => (a.runnerNumber ? -1 : 1) - (b.runnerNumber ? -1 : 1) || Number(a.runnerNumber) - Number(b.runnerNumber),
        );
    }
    ...
    static setPlaceDividends(placeDividends: PlaceDividend[]) {
        if (placeDividends) {
            placeDividends.sort((dividend1, dividend2) => {
                return (
                    this.sortPlaceDividends(dividend1.position, dividend2.position) ||
                    this.sortPlaceDividends(dividend1.runnerNumber, dividend2.runnerNumber)
                );
            });
        }
