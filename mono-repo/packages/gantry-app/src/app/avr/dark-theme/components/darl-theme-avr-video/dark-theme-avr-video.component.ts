...
@Component({
    selector: 'gn-dark-theme-avr-video',
    templateUrl: './dark-theme-avr-video.component.html',
    styleUrl: './dark-theme-avr-video.component.scss',
})
export class DarkThemeAvrVideoComponent implements AfterViewInit, OnDestroy {
    avrVideoUrl: string;
    rxPlayer: RxPlayer;
    isVideoPlaying: boolean = false;
    /**
     * * Retry is 9 because with each retry, we will wait for sometime which will be 1000*retryCount.
     * * So 9 retry means 1+2+3+4... 45 seconds, and we only need to show video for max 40 seconds.
     */
    maxRetryCount: number = 9;
    retryCount: number = 0;
    ...
    ngAfterViewInit(): void {
        this.eventFeedUrlService.eventFeedApiUrls$.subscribe((urls) => {
            const controllerId = this.routeDataService.getQueryParams()['controllerId'];
    ...
    errorEventListener(rxPlayer: RxPlayer) {
        rxPlayer.addEventListener('error', (err: any) => {
            this.retryLoadVideo();
            console.error(
                `RX Player video is STOPPED with following errorType: ${err.errorType}, errorStatus: ${err.status}, errorMessage: ${err.message}, errorCode: ${err.code}, url: ${err.url}`,
            );
