...
export class MethodOfVicotryService {
    ...
    setBetTitle(betName: string, boxingData: BoxingDataContent) {
        const byDecision = boxingData.contentParameters?.ByDecisionorTechnicalDecision?.split('|') ?? [];
        const koOrTko = boxingData.contentParameters?.KOorTKOorDisqualification?.split('|') ?? [];
