...
describe('HorseRacingMeetingResultsService', () => {
    ...
    it('when prepareResult called, result market - forecast, tricast, exacta, trifecta should be set correctly', () => {
        const result = service.prepareResult(mockData.meetingResultMap);
        let placeDividend = result.horseRacingMeetingResultsTable[0]?.placeDividends;
        placeDividend?.forEach((place) => {
            if (place?.runnerNumber) {
                if (place?.position == 'null' || place?.position == '' || place?.position == 'undefined') {
                    place.position = result.horseRacingMeetingResultsTable[0].runnerList?.find(
                        (selection) => Number(selection?.horseRunnerNumber) == Number(place?.runnerNumber),
                    )?.position;
                }
            }
        });
        ...
        expect(placeDividend[0]?.dividend).toBe('9.34');
        expect(result.horseRacingMeetingResultsTable[0].runnerList[0].horseRunnerNumber).toBe('1');
    });

    it('when prepareResult called, horse details should be correct', () => {
        const result = service.prepareResult(mockData.meetingResultMap);
        expect(result.horseRacingMeetingResultsTable[0].runnerList[0].horseName).toBe('HORSE1');
