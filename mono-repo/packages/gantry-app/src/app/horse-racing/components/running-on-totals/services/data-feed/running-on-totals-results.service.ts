...
export class RunningOnTotalsResultsService {
                ...
                (dataString) => {
                    return dataString.pipe(
                        map((dataString) => {
                            ...
                            const tempResult = new RunningTempResult();

                            if (parsedData.runningOnTotals) {
                                const runningOnTotals: RunningResultContent = Object.assign(new RunningResultContent(), parsedData);
    ...
    setUrl(url: string, snapShotDataTimeOut?: number) {
    ...
    UpdateRunningOnTotalsIfExists(tempResultSum: RunningTempResult, newTempResult: RunningTempResult) {
        ...
        const existingMeetingResult =
            tempResultSum?.result?.types?.get(newMeetingResult?.runningOnTotals?.rpCourseId) ||
            tempResultSum?.result?.types?.get(newMeetingResult?.runningOnTotals?.rpTrackId);
