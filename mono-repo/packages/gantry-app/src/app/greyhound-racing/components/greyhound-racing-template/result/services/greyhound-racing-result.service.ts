...
export class GreyhoundRacingResultService {
    ...
    data$ = combineLatest([this.racingContent$, this.greyHoundData$, this.eventResult$, this.virtualRaceImageService$]).pipe(
    ...
    createGreyhoundRacingResultPage(
        ...
        eventResult?.types?.forEach((meetingResultContent: MeetingResultContent) => {
            ...
            greyhoundRacingResultPage.eachWay = this.eachwayPositionsService.setEachWay(
    ...
    private setRunnersAndNonRunners(
        ...
        selections.forEach((selection) => {
            ...
            greyhoundDetails.price = selection?.startingPriceFraction?.endsWith('/1')
                ? selection?.startingPriceFraction?.substring(0, selection?.startingPriceFraction?.indexOf('/'))
            ...
            if (!!greyhoundDetails.greyhoundName && greyhoundDetails.greyhoundName.includes(RunnerReserve.Reserve)) {
            ...
            greyhoundDetails.greyhoundName = StringHelper.checkSelectionNameLengthAndTrimEnd(
                greyhoundDetails.greyhoundName,
