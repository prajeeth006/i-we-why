...
export class PostPickNbService {
    ...
    public setRacingPostTipImageByPositions(
        ...
        if (position) {
            greyhoundRacingPostTip.push(this.greyhoundRacingPostTipImage(position, greyHoundImageData));
        }

        return greyhoundRacingPostTip;
    }

    greyhoundRacingPostTipImage(runnerNo: string, imageData: GreyhoundStaticContent) {
    ...
    getPostPick(
        ...
        if (postPicks?.length > 0) {
            postPicks.forEach((postPick) => {
                ...
                if (runner) {
                    const runnerIndex = top3Positions.push(runner);
                    greyhoundRacingRunnersResult.greyhoundRacingPostTip = this.setRacingPostTipImageByPositions(
