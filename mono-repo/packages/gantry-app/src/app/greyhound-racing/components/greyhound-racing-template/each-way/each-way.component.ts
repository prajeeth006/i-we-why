...
import { GreyhoundRunner } from '../../../models/racing-content.model';
    ...
    ngOnChanges() {
        if ((this.isFullScreenType && this.isUKEvent && this.showPostPic) || (!this.isHalfScreenType && !this.isFullScreenType)) {
            if (!this.areCurrentPricesPresent && this.isForm && !this.isFullScreenType && !this.isNonRunner && !this.hasAnyReservedRunner) {
                ...
                this.areCurrentPricesPresent &&
                !this.isFullScreenType &&
                !this.isNonRunner &&
                !this.hasAnyReservedRunner &&
                this.racingPostTipOrder?.length > 0
            ) {
                this.showRightSideContent = true;
                return;
            } else if (
                !this.isAdditionalMarket &&
                this.isFullScreenType &&
                this.racingPostTipOrder?.length > 0 &&
                !this.showForm &&
                !this.isNonRunner &&
