...
import { GreyhoundRunner, RacingContentGreyhoundResult } from '../../../../models/racing-content.model';
...
export class GreyhoundRacingRunnersService extends BaseRacingTemplateService {
    ...
    public createGreyhoundRacingRunnersResult(
        ...
        if (racingContent?.postPick) {
            ...
            greyhoundRacingRunnersResult.napOrNb = this.postPickNbService.setPostPickNb(
        ...
        this.greyhoundRacingResultService.isUkEvent = greyhoundRacingRunnersResult.isUKEvent;
        ...
        const isApprochingTraps = this.isCheckApprochingTraps(event?.raceStage, greyHoundImageData);
        ...
        for (const runnerNumber in selections) {
            ...
            greyhoundRacingEntry.trapImage = this.postPickNbService.greyhoundRacingPostTipImage(runnerNumber, greyHoundImageData);

            const racingContentRunner: GreyhoundRunner | null | undefined = racingContent?.runners?.find((runner: GreyhoundRunner) => {
            ...
            greyhoundRacingEntry.comment =
                countryFlags == TypeFlagCode.Aus
                    ? ''
                    : !!racingContentRunner && !!racingContentRunner.comment
                      ? racingContentRunner?.comment.toUpperCase()
                      : '';
            greyhoundRacingEntry.last5Runs = racingContentRunner?.last5Runs?.split('').join('-');
            ...
            if (selection && selection?.selectionName?.includes(RunnerReserve.Reserve)) {
                ...
                greyhoundRacingEntry.greyhoundName = StringHelper.checkSelectionNameLengthAndTrimEnd(
                    selection?.selectionName?.split('(')[0],
                    SelectionNameLength.Eighteen,
                );
            }
            ...
            if (greyhoundRacingRunnersResult?.markets?.length > 0) {
                ...
                if (defaultMarketSelectionPrices) {
                    ...
                    greyhoundRacingEntry.pastPrice1Str = SportBookMarketHelper.getPriceStr(defaultMarketSelectionPrices[1]);
                    greyhoundRacingEntry.pastPrice2Str = SportBookMarketHelper.getPriceStr(defaultMarketSelectionPrices[2]);
                    greyhoundRacingEntry.prices[defaultMarketName] = SportBookMarketHelper.getPriceStr(defaultMarketSelectionPrices[0]);
                ...
                if (greyhoundRacingRunnersResult?.markets?.length > 1) {
                    for (let i = 1; i < greyhoundRacingRunnersResult?.markets?.length; i++) {
                        ...
                        if (!!selectionPrices && selectionPrices?.length > 0) {
                            greyhoundRacingEntry.prices[additionalMarketName] = SportBookMarketHelper.getPriceStr(selectionPrices[0]);
                        }
        ...
        const horseRunners: Array<GreyhoundRacingEntry> = [];
        greyhoundRacingRunnersResult.greyhoundRacingEntries.forEach((val) => horseRunners.push(Object.assign({}, val)));
        ...
        const horseWithRunners = horseRunners?.filter((x) => !x.nonRunner);
        ...
        this.postPickNbService.getPostPick(racingContent, horseRunners, top3Positions, greyhoundRacingRunnersResult, greyHoundImageData);
