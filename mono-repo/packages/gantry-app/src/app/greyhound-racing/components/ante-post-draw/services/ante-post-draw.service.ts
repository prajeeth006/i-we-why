...
import { AntePostDrawResult, AntePostDrawSelection } from '../../../models/ante-post-draw.model';
...
export class AntePostDrawService {
    ...
    data$ = combineLatest([this.sportBookService.data$, this.gantryCommonContent$, this.greyHoundData$]).pipe(
    ...
    prepareAntiPostDrawResult(sportBookResult: SportBookResult, gantryCommonContent: GantryCommonContent, greyhoundData: GreyhoundStaticContent) {
        const result = new AntePostDrawResult();
        if (!sportBookResult || sportBookResult?.events?.size <= 0) {
            return result;
        }
        const sportBookEvent = sportBookResult.events.get(parseInt(this.eventId));
        if (sportBookEvent) {
    ...
    private prepareResult(
        ...
        const selectionsSorted = selections.sort((a, b) => Number(a.runnerNumber) - Number(b.runnerNumber));

        result.selections = selectionsSorted
            .map((selection) => {
                if (selection?.hideEntry) return new AntePostDrawSelection();
                const antePostDrawSelection = new AntePostDrawSelection();
                const prices = SportBookMarketHelper.getPriceStr(SportBookSelectionHelper.getLatestPrice(selection));
                antePostDrawSelection.name = !selection.hideEntry
