...
import { GreyhoundStaticContent, TrapChallengeEvent, TrapChallengeEventSelection, TrapChallengeResult } from '../../../models/trap-challenge.model';
        ...
        result.events = [...sportBookResult.events.values()].map((sportBookEvent) => {
            ...
            trapChallengeEvent.selections = [...(eventFirstMarket?.selections?.values() ?? [])]
                .map((selection) => {
                    ...
                    trapChallengeEventSelection.price =
                        !selection?.hidePrice && selection.prices?.price![0]
                            ? selection.prices?.price[0]?.denPrice == 1
                                ? selection?.prices?.price[0]?.numPrice?.toString()
