...
export class DarkThemeGreyhoundMeetingResultsService {
    ...
    private setGreyhoundMeetingResultsTemplate(resultingContentList: Array<MeetingResultContent>): GreyhoundMeetingResultsTemplate {
        ...
        resultingContentList.forEach((meetingResultContent: MeetingResultContent) => {
            ...
            greyhoundMeetingResults.vacantTraps = this.getVacantTraps(meetingResultContent.resultingContent?.resultMarket?.listOfSelections);
    ...
    getVacantTraps(resultingContent: ResultSelection[]): string {
        const vacantTraps: number[] = [];
        resultingContent.forEach((selection) => {
            if (SportBookSelectionHelper.isNonRunner(selection.selectionName)) {
                vacantTraps.push(selection.runnerNumber);
            }
        });
        return vacantTraps?.toString()?.replaceAll(',', ', ')?.trim();
    }
    ...
    private setRunnerPositions(listOfSelections: Array<ResultSelection>, greyhoundMeetingResults: GreyhoundMeetingResults): GreyhoundMeetingResults {
        listOfSelections.forEach((runner: ResultSelection) => {
            ...
            runnerDetails.price = runner?.startingPriceFraction;
            greyhoundMeetingResults?.runnerList?.push(runnerDetails);
        });
    ...
    private setPageContent(meetingResult: GreyhoundMeetingResultsTemplate, racingContent: GreyhoundStaticContent): GreyhoundMeetingResultsTemplate {
        meetingResult = this.setEventIndicatorIcon(meetingResult, racingContent);
        meetingResult.greyhoundStaticContent = racingContent;
        return meetingResult;
    }

    private setEventIndicatorIcon(meetingResult: GreyhoundMeetingResultsTemplate, racingContent: GreyhoundStaticContent) {
        meetingResult.eventLogo = meetingResult?.isVirtualRace ? racingContent?.racingVirtualImage?.src : racingContent?.greyHoundRacingImage?.src;
        return meetingResult;
    }
