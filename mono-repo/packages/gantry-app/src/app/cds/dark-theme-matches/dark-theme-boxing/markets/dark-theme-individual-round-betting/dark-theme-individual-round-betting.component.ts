...
import { BoxingCdsContent, MultiMarket, RoundBettingDetails } from '../../../../matches/boxing-cds/models/boxing-cds-content.model';

@Component({
    selector: 'gn-dark-theme-individual-round-betting',
    templateUrl: './dark-theme-individual-round-betting.component.html',
    styleUrl: './dark-theme-individual-round-betting.component.scss',
...
export class DarkThemeIndividualRoundBettingComponent implements OnChanges {
    ...
    ngOnChanges(change: { [matchData: string]: SimpleChange }) {
        ...
        const getFullPointsIndex = this.roundValues?.indexOf(this.matchData?.content?.contentParameters?.OnPointsValue ?? '');
        const getRoundSelection =
            getFullPointsIndex != -1
                ? this.roundValues?.splice(this.roundValues?.indexOf(this.matchData?.content?.contentParameters?.OnPointsValue ?? ''), 1)[0]
                : '';
