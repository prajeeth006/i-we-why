...
import { _RxVirtualView, _RxVirtualViewContent, _RxVirtualViewObserver, _RxVirtualViewPlaceholder } from './model';
...
export class RxVirtualView implements AfterContentInit, _RxVirtualView, OnDestroy {
     ...
     */
    readonly startWithPlaceholderAsap = input(this.#config.startWithPlaceholderAsap, {
        transform: booleanAttribute,
    });
     ...
     */
    readonly placeholderStrategy = input<RxStrategyNames<string>>(this.#config.placeholderStrategy);

    /**
     * The strategy to use for rendering the content.
     */
    readonly contentStrategy = input<RxStrategyNames<string>>(this.#config.contentStrategy);

    /**
     * A function extracting width & height from a ResizeObserverEntry
     */
    readonly extractSize = input<(entry: ResizeObserverEntry) => { width: number; height: number }>(defaultExtractSize);
    ...
    readonly width = computed(() => (this.size().width ? `${this.size().width}px` : 'auto'));

    readonly height = computed(() => (this.size().height ? `${this.size().height}px` : 'auto'));

    readonly containment = computed(() => {
        if (!this.useContainment()) {
            return null;
        }
        return this.useContentVisibility() && this.#placeholderVisible() ? 'size layout paint' : 'content';
    });
    ...
    readonly minHeight = computed(() => {
        return this.keepLastKnownSize() && this.#placeholderVisible() ? this.height() : null;
    });
    readonly minWidth = computed(() => {
        return this.keepLastKnownSize() && this.#placeholderVisible() ? this.width() : null;
    });
    ...
    private showContent$(): Observable<EmbeddedViewRef<unknown>> {
        return this.#strategyProvider.schedule(
            () => {
                ...
                const tmpl = (this.#viewCache!.getContent(this) as EmbeddedViewRef<unknown>) ?? this.#content!.templateRef.createEmbeddedView({});
    ...
    private renderPlaceholder() {
        ...
        if (this.#placeholder) {
            const placeholderRef = this.#viewCache!.getPlaceholder(this) ?? this.#placeholder.templateRef.createEmbeddedView({});
