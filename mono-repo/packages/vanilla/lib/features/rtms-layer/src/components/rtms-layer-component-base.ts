...
import { NativeAppService, NavigationService, TimerService, ViewTemplateForClient, WINDOW, trackByProp } from '@frontend/vanilla/core';
...
@Directive()
export abstract class RtmsLayerComponentBase implements OnInit, OnChanges, OnDestroy {
    static metaData = {
        inputs: ['message', 'content'],
        outputs: ['closeEvent'],
    };

    @ViewChild('bcmcontainer') bcmOverlay: ElementRef;
    @ViewChild('bcmtnc') tncElement: ElementRef;
    ...
    protected constructor(protected timerService: TimerService) {}
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes.message?.currentValue) {
            ...
            if (this.message.content.useRewardsOverlay) {
                if (this.message.messageType === RtmsMessageType.BONUS_OFFER) {
                    this.message.sourceStatus = this.message.bonusSourceStatus;
                }
    ...
    getSectionTitle(sectionTitle: string): string {
        return (this.content.messages && this.content.messages[`GameSection.${sectionTitle.replace(' ', '')}`]) || sectionTitle;
    }
    ...
    tacExpanded() {
        ...
        this.tacOffsetTop = this.tncElement.nativeElement.offsetTop;

        this.timeoutId = this.timerService.setTimeout(() => {
            this.bcmOverlay.nativeElement.scrollTop = this.tacOffsetTop - 100;
        }, 100);
    }
    ...
    private handleSitecoreTncMsgConfiguration(messageDetails: NotificationMessage) {
        if (messageDetails.content.showManualTermsAndConditionsOnOverlay && messageDetails.content.overlayManualTermsAndConditions) {
