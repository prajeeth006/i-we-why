import { ChangeDetectionStrategy, Component, ElementRef, ViewEncapsulation, booleanAttribute, computed, input } from '@angular/core';
...
@Component({
    ...
    host: {
        '[class]': 'hostClass()',
        '[class.ds-badge-disabled]': 'disabled() ? true : null',
...
export class DsBadge {
    ...
    hostClass = computed(() => `ds-badge ds-badge-${this.size()} ds-badge-${this.variant()}`);

    constructor(public elementRef: ElementRef<HTMLElement>) {}

    public getAriaLabel(): string {
        const mainContent = this.elementRef.nativeElement.querySelector('.ds-badge-text')?.textContent?.trim();
