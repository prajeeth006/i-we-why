...
export async function getMixinUsageAuditOutput(componentStylesData: ComponentStylesData[], options: DsRunnerOptions): Promise<AuditOutput> {
    function displayValue(numberOfFiles: number): string {
        return `${pluralizeToken('component', numberOfFiles)} not using all the mixins`;
    }
...
export function getDSComponentMixinsUsage(
    ...
    return Promise.all(
        componentStylesData.map(({ componentFilePath, componentSelector, stylesContent }) =>
            assertComponentMixinsUsage(componentFilePath, componentSelector, stylesContent, options),
        ),
    );
}
...
export async function assertComponentMixinsUsage(
    ...
    { directory, variableImportPattern }: { directory: string; variableImportPattern: string },
): Promise<Issue> {
    // informative issue (component styles are OK)
    const issue = { source: { file: toUnixPath(file) } } satisfies Pick<Issue, 'source'>;

    const result = await getGeneratedStylesMixinsUsage(stylesContent, variableImportPattern, directory);
    ...
    if (result.unusedMixins.length) {
        return {
            ...issue,
            severity: 'error',
            message: `<b>${selector}</b> - Unused mixins: ${result.unusedMixins.map((x) => `<code style="color:red">${x}</code>`).join(', ')}`,
        } satisfies Issue;
    }
    ...
    return {
        ...issue,
        severity: 'info',
        message: `<b>${selector}</b> - Used mixins: ${result.usedMixins.map((x) => `<code style="color:green">${x}</code>`).join(', ')}`,
    } satisfies Issue;
}
