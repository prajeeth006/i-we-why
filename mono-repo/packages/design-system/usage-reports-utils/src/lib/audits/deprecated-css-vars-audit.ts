...
export async function getDeprecatedCssVarsAuditOutput(
    ...
    function displayValue(numberOfFiles: number): string {
        return `${pluralizeToken('component', numberOfFiles)} using deprecated css variables`;
    }
    ...
    const componentDeprecatedCssVarsUsage = componentStylesData.map(({ componentFilePath, componentSelector }) => {
        const componentVarsUsage = cssVarsUsage.find((x) => x.componentSelector === componentSelector);
        if (!componentVarsUsage) return;
        const deprecatedCssVarsUsage = componentUsedDeprecatedCssVars(componentVarsUsage.result.usedVars, deprecatedCssVars);
        return { componentFilePath, componentSelector, deprecatedCssVarsUsage };
    });

    const usedDeprecatedCssVars = componentDeprecatedCssVarsUsage.reduce((acc: string[], curr) => {
        if (curr) {
    ...
    const unusedDeprecatedCssVars = deprecatedCssVars.filter((x) => !usedDeprecatedCssVars.includes(x));

    const issues = await Promise.all(
        componentStylesData.map(({ componentFilePath, componentSelector }) => {
            const result = componentDeprecatedCssVarsUsage.find((x) => x && x.componentSelector === componentSelector);
            return assertDeprecatedCssVarsUsage(componentFilePath, componentSelector, result?.deprecatedCssVarsUsage || []);
        }),
    );
...
export function assertDeprecatedCssVarsUsage(file: string, selector: string, deprecatedCssVarsUsage: string[]): Issue {
    ...
    return {
        ...issue,
        severity: 'error',
        message: `<b>${selector}</b> - Using deprecated css variables: <span style="color: red">${deprecatedCssVarsUsage.join(', ').substring(0, 800)}</span>`,
    } satisfies Issue;
}

function componentUsedDeprecatedCssVars(cmpUsedCssVars: string[], allDeprecatedCssVars: string[]): string[] {
