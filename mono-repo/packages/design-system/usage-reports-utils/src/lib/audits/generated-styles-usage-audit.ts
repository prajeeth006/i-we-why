...
export const dsGeneratedStylesUsageAuditSlug = 'angular-ds-generated-styles-usage';

export async function getGeneratedStylesUsageAuditOutput(componentStylesData: ComponentStylesData[], options: DsRunnerOptions): Promise<AuditOutput> {
    function displayValue(numberOfFiles: number): string {
        return `${pluralizeToken('component', numberOfFiles)} not using generated styles`;
    }
    ...
    const issues = await getDSComponentGeneratedStyleFileUsage(componentStylesData, options);
...
export function getDSComponentGeneratedStyleFileUsage(
    ...
    return Promise.all(
        componentStylesData.map(({ componentFilePath, componentSelector, stylesContent }) =>
            assertComponentStylesFileUsage(componentFilePath, componentSelector, stylesContent, options),
        ),
    );
}
...
export function errorMessageNoUsageOfVariables(filePath: string, selector: string) {
