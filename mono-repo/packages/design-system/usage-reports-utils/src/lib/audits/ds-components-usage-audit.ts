...
export async function getDsComponentsUsageAuditOutput(directory: string): Promise<AuditOutput> {
...
export async function getSelectorsCount(filePath: string): Promise<DsComponent[]> {
...
function toRows(data: DsUsageData): TableRowObject[] {
    return data.components.map((o) => {
        return { source: data.file, selector: o.selector, component: o.component, count: o.count };
    });
}
