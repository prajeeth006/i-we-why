...
export async function getTokensOverrideAuditOutput(directory: string): Promise<AuditOutput> {
...
function displayValue(numberOfFiles: number): string {
    return `${pluralizeToken('token values', numberOfFiles)} were overridden manually on Design System components consumption`;
}
