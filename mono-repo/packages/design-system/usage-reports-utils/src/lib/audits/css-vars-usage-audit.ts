...
export async function getCssVarsUsageAuditOutput(
    ...
    const issues = await Promise.all(
        componentStylesData.map(({ componentFilePath, componentSelector }) => {
            ...
            return assertComponentCssVarsUsage(componentFilePath, componentSelector, componentVarsUsage.result);
        }),
    );

    function displayValue(numberOfFiles: number): string {
        return `${pluralizeToken('component', numberOfFiles)} not using all the css variables provided`;
    }
...
export async function assertComponentCssVarsUsage(file: string, selector: string, cssVarsUsage: DsCssVarsUsage): Promise<Issue> {
    ...
    if (cssVarsUsage.unusedVars.length) {
        return {
            ...issue,
            severity: 'error',
            message: `<b>${selector}</b> - Unused css variables: <span style="color: red">${cssVarsUsage.unusedVars.join(', ').substring(0, 800)}</span>`,
        } satisfies Issue;
    }
    ...
    return {
        ...issue,
        severity: 'info',
        message: `<b>${selector}</b> - Used css variables: <span style="color: green">${cssVarsUsage.usedVars.join(', ').substring(0, 800)}</span>`,
    } satisfies Issue;
}
