...
import { angularComponentRegex, angularComponentSelectorRegex, loadComponentStyles } from '../../utils';

export function getDSComponentStylesData(options: DsRunnerOptions): Promise<ComponentStylesData[]> {
    ...
    return crawlFileSystem({
        ...
        fileTransform: async (filePath: string) => {
            ...
            const isComponentFile = Boolean(componentContent.match(angularComponentRegex));
            if (!isComponentFile) {
                return false;
            }
            const selector = componentContent.match(angularComponentSelectorRegex)?.[3] || false;
            ...
            const cmpFilePath = replaceAll(filePath, '\\', '/').split('/').slice(0, -1).join('/');

            const componentStyles = await loadComponentStyles(componentContent, cmpFilePath);
