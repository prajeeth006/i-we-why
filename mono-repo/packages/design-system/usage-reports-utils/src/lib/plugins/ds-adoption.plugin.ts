...
import { dsComponentsHintAuditSlug, getClassToBeReplacedWithDsComponentAuditOutput } from '../audits/ds-components-hint-audit';
import { dsTokensOverrideAuditSlug, getTokensOverrideAuditOutput } from '../audits/ds-components-tokens-override-audit';
import { dsComponentsUsageAuditSlug, getDsComponentsUsageAuditOutput } from '../audits/ds-components-usage-audit';
...
export function create(options: DsAdoptionPluginOptions): PluginConfig {
    return {
        ...
        icon: 'javascript',
        description: 'A plugin to measure adoption of Design System components in project.',
        runner: () => runnerFunction(options),
        audits,
    };
}

export async function runnerFunction(options: DsAdoptionPluginOptions): Promise<AuditOutputs> {
    const dsComponentsUsage = await getDsComponentsUsageAuditOutput(options.directory);
