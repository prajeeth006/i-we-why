...
export const generatedStylesImportRegex = /@use\s+'([^']+)'(\w*\s+)?as\s+[\w,-]+;/g;
...
export const angularComponentSelectorRegex = /(selector)(\s*:\s*['"])([^'"]+)(['"])/m;
export const angularComponentStyleUrlsRegex = /styleUrl[s?]\s*:\s*[\[?]['"]([^'"]+)['"][]?]/m;
export const angularComponentStyleUrlRegex = /styleUrl\s*:\s*['"]([^'"]+)['"]/m;
export const angularComponentInlineStylesRegex = /styles:\s*\[\s*`([^`]+)`\s*]/m;

export async function loadComponentStyles(componentContent: string, cmpFilePath: string, root = process.cwd()): Promise<string | null> {
    ...
    if (externalStylePath) {
        const fileName = externalStylePath.startsWith('./') ? externalStylePath.replace('./', '') : externalStylePath;
        const styleFilePath = join(root, cmpFilePath, fileName);
        return readTextFile(styleFilePath);
    }

    const inlineStyles = componentContent.match(angularComponentInlineStylesRegex)?.[1] || false;
    if (inlineStyles) {
        return inlineStyles;
    }
...
export async function getGeneratedStylesMixinsUsage(
    ...
    const allStyles = await loadGeneratedStylesFromImports(componentStyles, variableImportPattern, uiFolderPath);

    const allMixinsWrapper: string[] = [];
    const allMixinsUsagesArray = allStyles.flatMap(({ alias, content }) => {
        const allMixins = Array.from(content.matchAll(mixinsNameRegex)).map((x) => x[1]);
        allMixins.forEach((x) => {
...
export async function getGeneratedStylesCssVarsUsage(
    ...
    const allVars = uniqueArray(Array.from(content.matchAll(cssVariablesRegex)).map((x) => x[0]));
    const allComponentCssVariables = uniqueArray(Array.from(componentStyles.matchAll(cssVariablesRegex)).map((x) => x[0]));
...
> {
    if (!stylesContent.includes(importPattern)) return Promise.resolve([{ content: '', alias: '' }]);
    ...
    if (scssImportStatements) {
        for (const scssImportStatement of scssImportStatements) {
            const { path, alias } = getPathAndAlias(scssImportStatement, importPattern);
            returnData.push({
...
function getPathAndAlias(scssImportText: string, importPattern: string) {
    ...
    const pathsToBeReplaced = ['../../../../../generated', '../../../../generated', '../../../generated', '../../generated'];

    for (const x of pathsToBeReplaced) {
        path = path.replace(x, 'generated'); // remove ../../ etc from path
    }

    const pathsToBeReplacedFont = ['../../../../../fonts', '../../../../fonts', '../../../fonts', '../../fonts'];
...
export async function getDeprecatedCssVarsFromFile(filePath: string): Promise<string[]> {
    return await readTextFile(filePath).then((res) => {
        return uniqueArray(res.split(/(\r\n|\r|\n)/).filter((x) => x.startsWith('--')));
    });
}
...
export function getSelectors(selectors: string[]) {
    return selectors.map((selector) => new RegExp(`(?<![\\w-])${selector}(?![\\w-])`, 'g'));
}
