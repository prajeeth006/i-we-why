...
export class MarketPriceTransformPipe implements PipeTransform {
  ...
  transform(price: string, marketName: string, horseRacingEntry: HorseRacingEntry, isPastPrice?: boolean, market?: SportBookMarketStructured, isRaceOff?: boolean, eventStatus?: string, eventDisplayStatus?: string, isVirtualEvent?: boolean): string {
    if (!isVirtualEvent) {
      if ((horseRacingEntry?.hideEntry && horseRacingEntry?.hideEntry[market?.marketName])
      ...
      if (isRaceOff || (eventStatus?.toUpperCase() == EventStatus.Active && market?.marketStatus?.toUpperCase() == MarketStatus.Active)) {
        let nonRunnerPrice = this.checkNRorWD(horseRacingEntry, isPastPrice, price);
        if (nonRunnerPrice)
          return nonRunnerPrice;
      }
      ...
      if (!(isRaceOff && (eventStatus?.toUpperCase() == EventStatus.Suspended && eventDisplayStatus?.toUpperCase() == DisplayStatus.Displayed))) {
        if (isRaceOff && market?.marketStatus?.toUpperCase() == MarketStatus.Suspended) {
          //Continue...
        } else if ((horseRacingEntry?.hidePrice && horseRacingEntry?.hidePrice[market?.marketName]) || market?.marketStatus?.toUpperCase() == MarketStatus.Suspended) {
          if ((horseRacingEntry?.hidePrice && horseRacingEntry?.hidePrice[market?.marketName]) || (eventStatus?.toUpperCase() == EventStatus.Suspended || market?.marketStatus?.toUpperCase() == MarketStatus.Suspended)) {
            if (horseRacingEntry.nonRunner && !isPastPrice) {
              return 'N/R';
            }
            if (horseRacingEntry.isWithdrawn && !isPastPrice) {
