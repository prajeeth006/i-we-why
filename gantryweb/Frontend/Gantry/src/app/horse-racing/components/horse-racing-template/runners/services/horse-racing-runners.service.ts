...
import { HorseRacingRunnersResult, HorseRacingEntry, RunnerImages } from 'src/app/horse-racing/models/horse-racing-template.model';
...
export class HorseRacingRunnersService extends BaseRacingTemplateService {
    ...
    private setRacingEntries(
        ...
        for (let runnerNumber in runnerNumberMarketNameSelectionPvk) {
            ...
            if (horseContent) {
                ...
                this.horseRacingEntry.horseName = `${StringHelper.checkSelectionNameLengthAndTrimEnd(horseContent.horseName, SelectionNameLength.Eighteen)?.toLocaleUpperCase()}`;
                this.horseRacingEntry.jockeyName = this.matchingSisWithdrawnSelectionStatus != null
                    ? horseRacingContent?.contentParameters?.Withdrawn
                    : horseContent.nonRunner
                        ? HorseRacing.NonRunner
                        : horseContent.isWithdrawn
                            ? horseRacingContent?.contentParameters?.Withdrawn
                            : `${!!horseContent.jockey ? horseContent.jockey?.toLocaleUpperCase() : ''}${horseContent.allowance ? ' (' + horseContent.allowance + ')' : ''}`
                this.horseRacingEntry.hasJockeyChanged = horseContent.hasJockeyChanged;
            ...
            if (marketNames.length > 0) {
                ...
                if (defaultMarketSelectionPrices) {
                    ...
                    this.horseRacingEntry.pastPrice1Str = SportBookMarketHelper.getPriceStr(defaultMarketSelectionPrices[1]);
                    this.horseRacingEntry.pastPrice2Str = SportBookMarketHelper.getPriceStr(defaultMarketSelectionPrices[2]);
                    this.horseRacingEntry.prices[defaultMarketName] = SportBookMarketHelper.getPriceStr(defaultMarketSelectionPrices[0]);
                ...
                for (let i = 1; i < marketNames?.length; i++) {
                    ...
                    if (selectionPrices?.length > 0) {
                        this.horseRacingEntry.prices[additionalMarketName] = SportBookMarketHelper.getPriceStr(selectionPrices[0]);
                    }
                ...
                for (let i = 0; i < marketNames?.length; i++) {
                    ...
                    if (selection?.selectionName?.includes('N/R')) {
                        ...
                        this.horseRacingEntry.jockeyName = HorseRacing.NonRunner;
                    }
                    if (selection?.selectionName?.includes('W/D') || this.matchingSisWithdrawnSelectionStatus?.status?.includes('W')) {
                        ...
                        this.horseRacingEntry.jockeyName = horseRacingContent?.contentParameters?.Withdrawn;
                    }
                    if (selection?.selectionName?.includes(HorseRacing.Reserve)) {
                        this.horseRacingEntry.isReserved = true;
                        this.horseRacingEntry.horseName = StringHelper.checkSelectionNameLengthAndTrimEnd(selection?.selectionName?.split("(")[0], SelectionNameLength.Eighteen);
                    }
    ...
    private setPriceInfos(horseRacingRunnersResult: HorseRacingRunnersResult) {
        ...
        horseRacingRunnersResult.arePastPricesPresent =
            horseRacingRunnersResult?.horseRacingEntries?.some(x => !!x.pastPrice1Str || !!x.pastPrice2Str);
    }
