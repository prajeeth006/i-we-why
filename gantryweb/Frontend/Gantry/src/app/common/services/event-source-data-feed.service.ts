...
export class EventSourceDataFeedService {
  ...
  getServerSentEvent(url: string, checkSuccessResponse: boolean = false, snapShotDataTimeOut: number = 2000, checkForSnapShot = true): Observable<any> {

    let connectionDetails =  new ConnectionDetails(url, checkSuccessResponse, snapShotDataTimeOut, checkForSnapShot);
    ...
    return new Observable((observer: Subscriber<string>) => {
      this.dataFeedAuthService.dataFeedAuthenticationApi$.subscribe({
        next: (authentication:DataFeedAuthentication) => {
          ...
          connectionDetails.authenticationKey = authentication.authenticationKey;
          connectionDetails.pendingConnectionRetryDelay = authentication.pendingConnectionRetryDelay ?? connectionDetails.pendingConnectionRetryDelay;
          connectionDetails.failedConnectionRetryDelay = authentication.failedConnectionRetryDelay ?? connectionDetails.failedConnectionRetryDelay;
  ...
  connectToSSE(connectionDetails: any,){
    ...
    connectionDetails.snapshotRetryTimeOutTimer = setTimeout(() => {
      if ((connectionDetails.eventSource?.readyState == EventSource.OPEN && connectionDetails.checkForSnapShot && !connectionDetails.snapShotReceived)
      ) {
        this.setDFConnectionStatus(connectionDetails, DataFeedStatus.DFConnectionTimeout);
        eventSource.close();
        this.connectToSSE(connectionDetails);
    ...
    eventSource.onerror = (errorEvent: any) => {
      ...
      if (errorEvent.currentTarget.readyState == EventSource.CLOSED) {
        if (connectionDetails.checkSuccessResponse) {
          this._zone.run(() => {
            if (!(this.urlStaleDataMap.has(connectionDetails.dfUrl) && this.urlStaleDataMap.get(connectionDetails.dfUrl))) {
              this.errorService.setError('Retrying to connect DF because of Connection lost/failed/timeout with event source : ' + connectionDetails.dfUrl);
            }
          });
        }
        ...
        setTimeout(() => {
          ...
          eventSource.close();
          this.connectToSSE(connectionDetails);
        }, connectionDetails.failedConnectionRetryDelay, this);
      } else {
        if (connectionDetails.checkSuccessResponse) {
          this._zone.run(() => {
            if (!(this.urlStaleDataMap.has(connectionDetails.dfUrl) && this.urlStaleDataMap.get(connectionDetails.dfUrl))) {
    ...
    switch(dataFeedStatus){
      ...
      case DataFeedStatus.DFConnectionTimeout:
        if (connectionDetails.eventSource?.readyState == EventSource.CONNECTING) {
          ...
          this.logError(connectionDetails.dfUrl, 'Retrying to connect DF because of Connection not established with DF with in ' + connectionDetails.pendingConnectionRetryDelay + ' Retrying to connect DF', 'ConnectionStatus', false, LogType.Information);
          console.info('Retrying to connect DF because of Connection not established with DF with in ' + connectionDetails.pendingConnectionRetryDelay + ' Retrying to connect DF', connectionDetails.dfUrl);
          if(connectionDetails.checkSuccessResponse && !connectionDetails.snapShotReceived){
            this.errorService.setError('Retrying to connect DF because of Connection not established with DF within ' + connectionDetails.pendingConnectionRetryDelay + " : " + connectionDetails.dfUrl);
          }
        }
        break;
      ...
      case DataFeedStatus.SnapShotReceived:
        ...
        let requestEndedAt: number = Date.now();
        if (!connectionDetails.snapShotReceived) {
          this.logError(connectionDetails.dfUrl, "Snapshot message received after " + (requestEndedAt - connectionDetails.requestStartedAt) + " Milliseconds.", "ConnectionStatus", true, LogType.Information);
          console.info('SnapShot message received after ' + (requestEndedAt - connectionDetails.requestStartedAt) + ' : ', responseData, connectionDetails.dfUrl, this.errorService.isSnapshotDataAvailable);
        }
  ...
  setDFErrorStatus(connectionDetails: any){
    if(connectionDetails.isSnapShotTimeOut){
      ...
      else if(connectionDetails.checkSuccessResponse && 
        connectionDetails.connectionStatus != DataFeedStatus.DFConnectionFailed && 
        connectionDetails.connectionStatus != DataFeedStatus.ConnectedToDF && 
      ...
      } else if(connectionDetails.checkForSnapShot && 
        connectionDetails.connectionStatus != DataFeedStatus.DFConnectionFailed && 
        !connectionDetails.has404Error && 
        !connectionDetails.snapShotReceived
      ){
        console.error("Snapshot data not received with in "+ connectionDetails.snapShotDataTimeOut + " Milliseconds.", connectionDetails.dfUrl);
        this.errorService.setError("Waiting for Snapshot data  : " + connectionDetails.dfUrl,);
        this.logError(connectionDetails.dfUrl, "Snapshot data not received with in "+ connectionDetails.snapShotDataTimeOut + " Milliseconds.", "-1", true);
