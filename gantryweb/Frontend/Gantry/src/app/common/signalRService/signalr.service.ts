...
export class SignalrService {
  ...
  constructor(private loggerService: LoggerService, private eventFeedUrlService: EventFeedUrlService,
    private routeDataService: RouteDataService, private errorService: ErrorService) {
    ...
    this.eventFeedUrlService.eventFeedApiUrls$.subscribe((eventFeedApiUrls: EventFeedApiUrls) => {
      ...
      if (!!window['label'] && !!queryParams['contentItemId']) {
        this.signalRUrl = `${eventFeedApiUrls.signalRHubMessageApi}?itemID=${queryParams['contentItemId']}&labelName=${window['label']}`;
  ...
  setTimeOutOnSignalRConnectionState() {
    this.signalRConnetionId = setTimeout(() => {
      ...
      if (!!this.connection && this.connection?.state !== signalR.HubConnectionState.Connected) {
        let signalRConnectionMessage = `SignalR connection still pending. Connection Status is ${this.connection.state}`;
  ...
  parseSignalRMessage(signalRMessage: string) {
    try {
      ...
      let formData = sitecoreItemRoot?.Item?.Fields?.find((x: any) => x.Key == eventFormData)?.Content;
