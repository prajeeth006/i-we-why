...
export class SportBookEventHelper {
    ...
    static getPriceHeader(
        ...
        sisData: SisData,
        isVirtualEvent: boolean): string {

        if (!isVirtualEvent) {
            if (!raceStage && !event.offTime) {
                return gantryCommonContent?.contentParameters?.CoralPrice;
            }

            const offTimeDate = new Date(event.offTime);
            const offTimeText = `${gantryCommonContent?.contentParameters?.RaceOff} @ ${offTimeDate.getHours().toString().padStart(2, "0")}:${offTimeDate.getMinutes().toString().padStart(2, "0")}:${offTimeDate.getSeconds().toString().padStart(2, "0")}`;
            ...
            return gantryCommonContent?.contentParameters?.LiveShow;
        }
        else {
            if (!!event.offTime) {
                const offTimeDate = new Date(event.offTime);
    ...
    static eventStartTimeAddedInTypeName(event: SportBookEventStructured) {
        ...
        let eventStartTime = StringHelper.getBtcTime(event?.eventDateTime);
        let eventName = event?.eventName?.trim().substring(event?.eventName?.trim().indexOf(' '));
