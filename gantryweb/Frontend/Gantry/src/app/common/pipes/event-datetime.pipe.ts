...
export class EventDatetimePipe implements PipeTransform {
  ...
  transform(value: string, eventDateTime: Date, gantryCommonContent: SportContentParameters, timeStampOptions?: Intl.DateTimeFormatOptions, isGolf: boolean = false): string {
  ...
  private getTodayAndTomorrowDates(finalTimeStampOptions: Intl.DateTimeFormatOptions, eventDateTime: Date, todayOrTomorrow: string): string {
    let eventDate = finalTimeStampOptions.weekday == TimeStampOptions.weekday ? eventDateTime?.toLocaleString('en-us', { timeStyle: finalTimeStampOptions.timeStyle }) : todayOrTomorrow;
    return eventDate;
  }
